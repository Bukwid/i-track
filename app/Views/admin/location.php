        ...
        <header class="flex justify-between items-center p-6 lg:p-8 bg-slate-50 sticky top-0 z-20">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm lg:text-base text-slate-500 hidden sm:block">Manage campus locations</p>
                </div>
            </div>
        </header>
        ...
        <div class="p-6 lg:p-8 pt-0 flex-1 w-full max-w-full overflow-hidden">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden flex flex-col w-full">
                <div class="p-6 md:p-8 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <h3 class="text-xl font-bold text-slate-800">Location List</h3>
                    <a href="<?= base_url('location/add') ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-medium shadow-lg shadow-blue-200 transition-all active:scale-95 text-sm flex items-center gap-2 justify-center w-full sm:w-auto">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-slate-400 text-sm uppercase tracking-wider font-medium border-b border-slate-100">
                            <th class="pb-4 font-semibold px-2">Location Code</th>
                            <th class="pb-4 font-semibold px-2">Name</th>
                            <th class="pb-4 font-semibold px-2">Building</th>
                        ...
                        <?php if(!empty($locationList)): ?>
                            <?php foreach($locationList as $location): ?>
                                <tr class="hover:bg-slate-50/80 transition-colors group">
                                    <td class="py-4 px-2 font-medium text-slate-700"><span class="bg-slate-100 text-slate-600 px-2 py-1 rounded font-mono text-xs border border-slate-200"><?= esc($location['location_code']) ?></span></td>
                                    <td class="py-4 px-2 text-slate-700 font-medium"><?= esc($location['name']) ?></td>
                                    ...
                                    <td class="py-4 px-2">
                                        <div class="flex flex-col xl:flex-row gap-2">
                                            <a href="<?= base_url('location/get-qrcode/' . $location['location_id']) ?>" class="bg-emerald-50 text-emerald-600 hover:bg-emerald-100 hover:text-emerald-700 px-4 py-2 rounded-lg text-xs font-semibold transition-colors text-center inline-flex items-center justify-center gap-1.5"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm14 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path></svg> QR Code</a>
                                            <a href="<?= base_url('location/delete/' . $location['location_id']) ?>" class="bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 px-4 py-2 rounded-lg text-xs font-semibold transition-colors text-center inline-flex items-center justify-center gap-1.5"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg> Delete</a>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
