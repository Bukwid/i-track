...
<head>
    ...
    <title>I-Track | Faculty Schedule</title>
    ...
    <main class="flex-1 flex flex-col overflow-y-auto w-full">
        ...
        <header class="flex justify-between items-center p-6 lg:p-8 bg-slate-50 sticky top-0 z-20">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl lg:text-3xl font-bold text-slate-800"><?= esc($faculty['first_name'] . ' ' . $faculty['last_name']) ?></h1>
                    <p class="text-sm lg:text-base text-slate-500 hidden sm:block">Faculty schedule management</p>
                </div>
            </div>
        </header>
        ...
        <div class="p-6 lg:p-8 pt-0 flex-1 w-full max-w-full overflow-hidden">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden flex flex-col w-full">
                <div class="p-6 md:p-8 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h3 class="text-xl font-bold text-slate-800"><?= esc($faculty['first_name'] . ' ' . $faculty['last_name']) ?>'s Schedule</h3>
                        <p class="text-sm text-slate-500 mt-1">View and manage assigned schedules.</p>
                    </div>
                    <a href="<?= base_url('faculty/addSchedule/' . $faculty['faculty_id']) ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-medium shadow-lg shadow-blue-200 transition-all active:scale-95 text-sm flex items-center gap-2 justify-center w-full sm:w-auto">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="text-slate-400 text-sm uppercase tracking-wider font-medium border-b border-slate-100">
                                <th class="pb-4 font-semibold px-2">Day</th>
                                <th class="pb-4 font-semibold px-2">Start Time</th>
                                <th class="pb-4 font-semibold px-2">End Time</th>
                                <th class="pb-4 font-semibold px-2">Subject</th>
                                <th class="pb-4 font-semibold px-2">Location</th>
                        ...
                        <tbody class="text-sm divide-y divide-slate-50">
                            <?php if(!empty($schedules)): ?>
                                <?php foreach($schedules as $schedule): ?>
                                    <tr class="hover:bg-slate-50/80 transition-colors group">
                                        <td class="py-4 px-2 font-medium text-slate-700">
                                            <span class="bg-blue-50 text-blue-600 px-2.5 py-1 rounded-lg text-xs font-semibold"><?= esc($schedule['day_of_week']) ?></span>
                                        </td>
                                        <td class="py-4 px-2 text-slate-600 font-mono text-xs"><?= esc($schedule['start_time']) ?></td>
                                        <td class="py-4 px-2 text-slate-600 font-mono text-xs"><?= esc($schedule['end_time']) ?></td>
                                        <td class="py-4 px-2 text-slate-700 font-medium"><?= esc($schedule['subject_name']) ?></td>
                                        <td class="py-4 px-2 text-slate-600">
                                            <div class="flex items-center gap-2">
                                                <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                                                <?= esc($schedule['location_name'] . ' - ' . $schedule['building']) ?>
                                            </div>
                                        </td>
                                        <td class="py-4 px-2">
                                            <a href="<?= base_url('faculty/deleteSchedule/' . $schedule['schedule_id']) ?>" class="bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 px-4 py-2 rounded-lg text-xs font-semibold transition-colors inline-flex items-center gap-1.5">
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="6" class="py-12 text-center text-slate-500 bg-slate-50/50 rounded-2xl border-dashed border-2 border-slate-100 mt-4">
