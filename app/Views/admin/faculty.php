        ...
        <div class="p-8">
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                    <h3 class="font-bold text-gray-700">Faculty List</h3>
                    <a href="<?= base_url('faculty/add') ?>" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700">+ Add Faculty</a>
                </div>
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        <?php if(!empty($facultyList)): ?>
                            <?php foreach($facultyList as $faculty): ?>
                                <tr>
                                    <td class="p-4 font-medium"><?= esc($faculty['last_name']) ?></td>
                                    <td class="p-4 text-gray-600"><?= esc($faculty['first_name']) ?></td>
                                    <td class="p-4 text-gray-600"><?= esc($faculty['username']) ?></td>
                                    <td>
                                        <a href="<?= base_url('faculty/schedule/' . $faculty['faculty_id']) ?>" class="bg-yellow-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-yellow-700">View Schedule</a>
                                        <a href="<?= base_url('faculty/delete/' . $faculty['faculty_id']) ?>" class="bg-red-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-red-700">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="4" class="p-4 text-center text-gray-500">No faculty members found.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
