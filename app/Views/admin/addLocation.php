...
<head>
    ...
    <title>I-Track | Add Location</title>
...
<body class="bg-slate-50 flex h-screen overflow-hidden text-slate-800">
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-64 transform -translate-x-full lg:translate-x-0 lg:static flex flex-col bg-slate-900 text-white transition-transform duration-300 ease-in-out shadow-xl lg:shadow-none">
        ...
        <nav class="flex-1 px-4 space-y-2 mt-6 overflow-y-auto">
            ...
            <a href="<?= base_url('location') ?>" class="flex items-center gap-3 p-3 rounded-xl transition-all bg-blue-600 shadow-md shadow-blue-900/20 text-white">
    ...
    <main class="flex-1 flex flex-col overflow-y-auto">
        <header class="flex justify-between items-center p-6 lg:p-8 bg-slate-50 sticky top-0 z-20">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl lg:text-3xl font-bold text-slate-800">Add Location</h1>
                    <p class="text-sm lg:text-base text-slate-500 hidden sm:block">Register a new campus location for tracking</p>
                </div>
            </div>
        </header>
        ...
        <div class="p-6 lg:p-8 pt-0 flex-1">
            <div class="max-w-3xl mx-auto">
                <nav class="text-sm text-slate-500 mb-6 flex items-center gap-2">
                    ...
                    <span class="text-slate-800 font-medium">Add New Location</span>
                </nav>

                <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                    <div class="p-6 md:p-8 border-b border-slate-100">
                        <h3 class="text-xl font-bold text-slate-800">Location Registration</h3>
                        <p class="text-sm text-slate-500 mt-1">Enter the details below to create a new location for I-Track.</p>
                    </div>

                    <form action="<?= base_url('location/add') ?>" method="POST" class="p-6 md:p-8">
                        <?= csrf_field() ?>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                <label for="location_code" class="block text-sm font-semibold text-slate-700 mb-2">Location Code</label>
                            ...
                            <div>
                                <label for="name" class="block text-sm font-semibold text-slate-700 mb-2">Location Name</label>
                        ...
                        <div class="mb-8">
                            <label for="building" class="block text-sm font-semibold text-slate-700 mb-2">Building</label>
                            <select name="building" id="building" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition bg-slate-50/50" required>
                                <option value="SHS Building 1" <?= old('building') == 'SHS Building 1' ? 'selected' : '' ?>>SHS Building 1</option>
                                <option value="SHS Building 2" <?= old('building') == 'SHS Building 2' ? 'selected' : '' ?>>SHS Building 2</option>
                                <option value="SHS Building 3" <?= old('building') == 'SHS Building 3' ? 'selected' : '' ?>>SHS Building 3</option>
                                <option value="SHS Building 4" <?= old('building') == 'SHS Building 4' ? 'selected' : '' ?>>SHS Building 4</option>
                                <option value="SHS Building 5" <?= old('building') == 'SHS Building 5' ? 'selected' : '' ?>>SHS Building 5</option>
                            </select>
                        </div>

                        <div class="flex items-center justify-end gap-4 border-t border-slate-100 pt-6">
                            <a href="<?= base_url('location') ?>" class="px-6 py-2.5 text-sm font-medium text-slate-500 hover:text-slate-800 transition rounded-xl hover:bg-slate-100">Cancel</a>
